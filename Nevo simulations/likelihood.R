...
loglik_wrapper <- function(par, data){
  ...
  alpha <- data.frame(alpha1=par[6], alpha2=par[7], alpha3=par[8])
...
loglik <- function(data, beta, alpha){
  ...
  prob_0000 <- ((1-data$y1) * (1-data$y2))*log(1-coefs$pi1_0-coefs$pi2_0+coefs$pi12_0)
  prob_0001 <- ((1-data$y1) * data$y2)*log(coefs$pi2_0-coefs$pi12_0)
  prob_0010 <- (data$y1 * (1-data$y2))*log(coefs$pi1_0-coefs$pi12_0)
  prob_0011 <- data$y1*data$y2*log(coefs$pi12_0)
  prob_1000 <- ((1-data$y1) * (1-data$y2))*log(1-coefs$pi1_1-coefs$pi2_1+coefs$pi12_1)
  prob_1001 <- ((1-data$y1) * data$y2)*log(coefs$pi2_1-coefs$pi12_1)
  prob_1010 <- (data$y1 * (1-data$y2))*log(coefs$pi1_1-coefs$pi12_1)
  prob_1011 <- data$y1*data$y2*log(coefs$pi12_1)
  
  sum((1-data$y1_prev)*(prob_0011+prob_0010+prob_0001+prob_0000) + (data$y1_prev)*(prob_1000+prob_1001+prob_1011+prob_1010))
}
...
pi1 <- function(beta1, X, y1, beta1y, alpha1){
...
gen_data <- function(n=5000, n_obs=10){
  ...
  # Known coefficients
  beta <- tibble(beta1=1.3, beta2=-0.5, betat=1, beta1y=0.8, beta2y=-1.3)
  alpha <- tibble(alpha1=-1, alpha2=-0.5, alpha3=0.5)
...
dat <- gen_data()
max_like <- optim(par=c(0.3,-0.5,1,0.8,-1.3,-1,-0.5,1),fn=loglik_wrapper,data=dat)


# Roadmap of dissertation
# Extending the 10's to incorporate cost - WORK ON THIS 
